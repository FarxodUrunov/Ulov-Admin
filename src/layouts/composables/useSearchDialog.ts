import { type WritableComputedRef, onMounted, onUnmounted } from 'vue'
import { useDebouncedRef } from '@/shared/composables/useDebouncedRef'

export const useSearchDialog = ({
  isDialogOpen
}: {
  isDialogOpen: WritableComputedRef<boolean>
}) => {
  const shorcuts: Array<{ label: string; icons: string[] }> = [
    {
      label: 'to select',
      icons: [
  ...
  const itemsGroup = [
    {
      heading: 'Essentials',
      items: ['Import Extension', 'Clipboard History', 'Manage Extensions', 'Options: Composition']
    },
    {
  ...
  const getItemIndex = (groupIndex: number, itemIndex: number) => {
    ...
    const prevGroupItem = itemsGroup[groupIndex - 1]
    if (Array.isArray(prevGroupItem?.items)) {
      index += prevGroupItem.items.length
    }
    return index
  }
