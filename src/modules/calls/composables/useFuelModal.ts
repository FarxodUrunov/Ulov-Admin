...
export const useFuelForm = () => {
  const formSchema = object({
    ...
    fuel_grade: number().required('formErrors.fieldRequired'),
    quantity: number().required('formErrors.fieldRequired'),
    tel_number: string()
      .matches(/^[+]998[0-9]{9}$/, {
  ...
  const fuelGrade = useField<FormType['fuel_grade']>('fuel_grade')
  const quantity = useField<FormType['quantity']>('quantity')
