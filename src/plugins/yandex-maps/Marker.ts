...
import type { MarkerType, MarkerFeature, RecursiveArray } from './types'
  ...
  setup(props, { emit, slots, expose }) {
    ...
    const balloonContentLayout: any = slots.component?.().length
      ? ymaps.templateLayoutFactory.createClass(
          `<div id="balloon-${props.markerId}" class="yandex-balloon"><div>`,
          {
            build() {
    ...
    if (slots.component?.().length) {
      return () =>
        isBalloonOpen.value &&
        h(Teleport, { to: `#balloon-${props.markerId}` }, [slots.component?.()])
    }
